...
from server.custom_layers import Scale


def DenseNet(nb_dense_block=4, growth_rate=48, nb_filter=96, reduction=0.5, dropout_rate=0.0, weight_decay=1e-4, classes=1000, weights_path=None):
    ...
    x = Conv2D(nb_filter, 7, 7, subsample=(2, 2),
...
def conv_block(x, stage, branch, nb_filter, dropout_rate=None, weight_decay=1e-4):
    ...
    x = Conv2D(inter_channel, 1, 1,
               name=conv_name_base+'_x1', bias=False)(x)
    ...
    x = Conv2D(nb_filter, 3, 3, name=conv_name_base +
               '_x2', bias=False)(x)
...
def transition_block(x, stage, nb_filter, compression=1.0, dropout_rate=None, weight_decay=1E-4):
    ...
    x = Conv2D(int(nb_filter * compression), 1, 1,
               name=conv_name_base, bias=False)(x)
...
def dense_block(x, stage, nb_layers, nb_filter, growth_rate, dropout_rate=None, weight_decay=1e-4, grow_nb_filters=True):
    ...
    for i in range(nb_layers):
        ...
        concat_feat = merge([concat_feat, x], mode='concat',
                            concat_axis=concat_axis, name='concat_'+str(stage)+'_'+str(branch))
